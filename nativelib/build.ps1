...
$CxxCompiler = if ($CxxCompiler) { $CxxCompiler } elseif ($Compiler -eq "gcc") { "g++" } elseif ($Compiler -eq "clang") { "clang++" } else { $Compiler }
...
if (Test-Path $webuiVendor) {
    ...
    if ($env:OS -eq "Windows_NT") {
        ...
        & $Compiler -std=c11 -I (Join-Path $repo "src") -I $webuiInclude -c (Join-Path $PSScriptRoot "webui/webui_little.c") -o $objects[0]
        if ($LASTEXITCODE -ne 0) { throw "Native WebUI wrapper build failed with exit code $LASTEXITCODE" }

        & $Compiler -std=c11 -I $webuiInclude $webuiDefines -c (Join-Path $webuiVendor "src/webui.c") -o $objects[1]
        if ($LASTEXITCODE -ne 0) { throw "WebUI core build failed with exit code $LASTEXITCODE" }

        & $Compiler -std=c11 -w -I $webuiInclude $webuiDefines -c (Join-Path $webuiVendor "src/civetweb/civetweb.c") -o $objects[2]
        ...
        $webuiFlags = @("-std=c11", "-shared", "-fPIC", "-I", (Join-Path $repo "src"), "-I", $webuiInclude) + $webuiDefines
